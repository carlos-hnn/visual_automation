...
def health_percent(frame: Frame, minimum_row_coverage: float = 0.18) -> float:
    """Estimate health from the bright red fill in a cropped vertical health bar."""
    hsv = cv2.cvtColor(frame.image, cv2.COLOR_BGR2HSV)
    lower_red = cv2.inRange(
        hsv,
        np.array((0, 120, 95), dtype=np.uint8),
        np.array((12, 255, 255), dtype=np.uint8),
    )
    upper_red = cv2.inRange(
        hsv,
        np.array((168, 120, 95), dtype=np.uint8),
...
def fish_templates(directory: Path) -> list[Path]:
    paths = sorted(path for path in directory.glob("*.png") if path.is_file())
    if not paths:
        raise FileNotFoundError(f"No fish PNG templates found in: {directory}")
    return paths
...
def run(
    *, monitor: int, bar_region: dict[str, int], inventory_region: dict[str, int],
    ...
    templates = fish_templates(templates_dir)
    stop_keys = StopKeys()
    mouse = build_mouse(0.16, 0.32, spot_jitter_pixels=spot_jitter)
    low_readings = 0
    last_click = float("-inf")

    print(f"{'DRY RUN' if dry_run else 'LIVE'}: checking health every {check_seconds:g}s; eat below {threshold_percent:g}%")
    ...
    try:
        with ScreenCapture(monitor=monitor) as screen:
            while not stop_keys.stop_requested:
                percent = health_percent(screen.capture(bar_region))
                low_readings = low_readings + 1 if percent < threshold_percent else 0
                print(f"health={percent:.1f}% low_readings={low_readings}/{required_low_readings}")
...
def main() -> int:
    ...
    pre_parser.add_argument("--config", type=Path, default=ROOT / "config" / "health_fish.example.json")
    known, _ = pre_parser.parse_known_args()
    config = load_json_config(known.config)

    parser = argparse.ArgumentParser(description="Monitor the health bar and eat fish below a threshold.", parents=[pre_parser])
    ...
    parser.add_argument("--templates-dir", type=Path, default=value_from_config(config, "templates_dir", "templates/health_fish"))
    parser.add_argument("--threshold-percent", type=float, default=value_from_config(config, "threshold_percent", 50.0))
    parser.add_argument("--check-seconds", type=float, default=value_from_config(config, "check_seconds", 1.0))
    parser.add_argument("--required-low-readings", type=int, default=value_from_config(config, "required_low_readings", 2))
    parser.add_argument("--cooldown-seconds", type=float, default=value_from_config(config, "cooldown_seconds", 3.0))
