...
function renderFields(){const s=state.scripts.find(x=>x.id===selected);document.getElementById('title').textContent=s.label;const box=document.getElementById('fields');box.innerHTML='';for(const [key,value] of Object.entries(s.config)){const d=document.createElement('div');const nested=typeof value==='object'&&value!==null;d.className='field'+(nested?' wide':'');const l=document.createElement('label');l.textContent=key;d.appendChild(l);let el;if(typeof value==='boolean'){el=document.createElement('input');el.type='checkbox';el.checked=value}else if(nested){el=document.createElement('textarea');el.value=JSON.stringify(value,null,2)}else{el=document.createElement('input');el.type=typeof value==='number'?'number':'text';if(typeof value==='number')el.step='any';el.value=value}el.dataset.key=key;el.dataset.type=Array.isArray(value)?'array':typeof value;d.appendChild(el);box.appendChild(d)}updateButtons()}
...
async function start(){const config=collect();if(config.dry_run===false&&!confirm('Live mode is enabled. This automation will move and click the mouse. Start it?'))return;await api('/api/start',{method:'POST',body:JSON.stringify({id:selected,config})});state.scripts.find(x=>x.id===selected).config=config;await refreshStatus()}
