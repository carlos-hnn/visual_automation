...
from visual_automation.game_states.gem_cutting import GemCuttingState
        ...
        with ScreenCapture(monitor=args.monitor) as screen:
            ...
            while not stop_keys.stop_requested and (args.loops <= 0 or completed < args.loops):
                ...
                if gem_template is None or gem_match is None:
                    ...
                    break
                clicks.click_match(gem_template, gem_match, gem_scale)
                wait_action("after gem withdrawal", args, args.dry_run)
                if not bank.close():
                    continue
                wait_action("after bank close", args, args.dry_run)
                if not clicks.find_and_click(templates["chisel"]):
                    continue
                ...
                if inventory_gem is None or inventory_match is None:
                    ...
                    continue
                clicks.click_match(inventory_gem, inventory_match, inventory_scale)
                if not clicks.find_and_click(confirmation):
