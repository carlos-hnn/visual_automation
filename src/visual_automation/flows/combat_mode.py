...
from visual_automation.game_states.combat import detect_combat_activity, detect_health_status, detect_prayer_status
...
def best_marked_inventory_item(
    frame: Frame,
    raw_mask: np.ndarray,
    ...
    for component_label in range(1, count):
        x, y, width, height, _area = (int(item) for item in stats[component_label])
        green_pixels = int(np.count_nonzero(raw_mask[labels == component_label]))
    ...
    return TaggedConsumable(label, candidates[0])
...
def best_red_marked_inventory_item(
    ...
    frame = screen.capture(inventory_region)
    return best_marked_inventory_item(
        frame, red_mask(frame.image), min_red_pixels, min_dimension,
        max_dimension, grouping_pixels, "red food marker",
    )
...
def detect_red_targets(
    ...
    max_dimension: int,
    grouping_pixels: int,
    anchor_exclusion_radius: float = 0.0,
    ...
    for label in range(1, count):
        ...
        if anchor_exclusion_radius > 0 and distance < anchor_exclusion_radius:
            continue
...
def run(
    *, monitor: int, window_title: str, combat_region_relative: dict[str, int],
    ...
    prayer_region_relative: dict[str, int],
    potion_marker_hsv_min: tuple[int, int, int], potion_marker_hsv_max: tuple[int, int, int],
    potion_marker_min_green_pixels: int, potion_marker_min_dimension: int,
            ...
            while not stop_keys.stop_requested:
                ...
                if now >= next_health_check and health_monitor_enabled:
                    ...
                    if (
                        ...
                        candidate = best_red_marked_inventory_item(
                            screen,
                            inventory_region,
                            food_marker_min_red_pixels,
                            food_marker_min_dimension,
                            food_marker_max_dimension,
                            food_marker_grouping_pixels,
                        )
                        if candidate is None:
                            print("no red food marker found")
                            report_failure("combat:food")
                        else:
                            report_progress("combat:food")
                            food_x, food_y = match_click_coordinates(candidate.match, 1.0, spot_jitter)
                            if dry_run:
                                print(
                                    f"would eat {candidate.label} pixels={candidate.match.score:.0f} "
                                    f"at=({food_x},{food_y})"
                                )
                            else:
                                mouse.click(food_x, food_y)
                                ...
                                print(
                                    f"ate {candidate.label} pixels={candidate.match.score:.0f} "
                                    f"at=({food_x},{food_y})"
                                )
                            last_eat = now
                            low_health_readings = 0
                ...
                if now >= next_health_check and prayer_monitor_enabled:
                    ...
                    if (
                        ...
                        candidate = best_green_marked_inventory_item(
                            screen,
                            inventory_region,
                            potion_marker_hsv_min,
                            potion_marker_hsv_max,
                            potion_marker_min_green_pixels,
                            potion_marker_min_dimension,
                            potion_marker_max_dimension,
                            potion_marker_grouping_pixels,
                            "green prayer potion marker",
                        )
                        if candidate is None:
                            print("no green prayer potion marker found")
                            report_failure("combat:prayer-potion")
                        else:
                            report_progress("combat:prayer-potion")
                            potion_x, potion_y = match_click_coordinates(candidate.match, 1.0, spot_jitter)
                            if dry_run:
                                print(
                                    f"would drink {candidate.label} pixels={candidate.match.score:.0f} "
                                    f"at=({potion_x},{potion_y})"
                                )
                            else:
                                ...
                                mouse.move_to(park_x, park_y)
                                print(
                                    f"drank {candidate.label} pixels={candidate.match.score:.0f} "
                                    f"at=({potion_x},{potion_y})"
                                )
                            last_drink = now
                            low_prayer_readings = 0
                ...
                if in_combat:
                    ...
                    if out_of_combat_readings >= required_out_of_combat_readings and not pending and not consumed_this_tick:
                        ...
                        if not targets:
                            ...
                            report_progress("combat:slayer-target")
                            target = targets[0]
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Enable or disable automatic red-tagged food eating",
    )
    parser.add_argument(
    ...
    try:
        ...
        return run(
            ...
            potion_marker_hsv_min=parse_hsv_triplet(value_from_config(config, "potion_marker_hsv_min", [35, 110, 70]), "potion_marker_hsv_min"),
            potion_marker_hsv_max=parse_hsv_triplet(value_from_config(config, "potion_marker_hsv_max", [90, 255, 255]), "potion_marker_hsv_max"),
            potion_marker_min_green_pixels=max(1, int(value_from_config(config, "potion_marker_min_green_pixels", 120))),
            potion_marker_min_dimension=max(1, int(value_from_config(config, "potion_marker_min_dimension", 8))),
            potion_marker_max_dimension=max(1, int(value_from_config(config, "potion_marker_max_dimension", 70))),
            potion_marker_grouping_pixels=max(0, int(value_from_config(config, "potion_marker_grouping_pixels", 2))),
