...
from v2.flows.woodcutting import click_marker, nearest_to_center, region_center, wait_ticks
from v2.game_states.bank import detect_bank_status
from v2.game_states.color_markers import capture_color_markers, find_color_markers, marker_settings_from_config
...
from v2.platforming import add_platform_argument, resolve_path, resolve_platform
from v2.template_config import click_offset_for, region_for, resolve_regions, scales_for, threshold_for

install_timestamped_print()

DEFAULT_CONFIG_PATH = ROOT / "config" / "wc-fossil.example.json"
...
def run_flow(config: dict[str, Any], args) -> int:
    ...
    print(f"{'DRY RUN' if args.dry_run else 'LIVE'}: wc-fossil; cycles={'until stopped' if args.loops <= 0 else args.loops}")
    ...
    try:
        with ScreenCapture(monitor=args.monitor) as screen:
            ...
            while not stop_keys.stop_requested and (args.loops <= 0 or completed < args.loops):
                ...
                completed += 1
                print(f"wc-fossil cycle complete: {completed}")
    finally:
        stop_keys.stop()
