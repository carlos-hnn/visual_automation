...
def wait_ticks(label: str, ticks: float, args, dry_run: bool) -> None:
    ...
    print(f"{label}: waiting {ticks:g} ticks, {delay:.2f}s")
    if not dry_run:
        time.sleep(delay)
    ...
    try:
        with ScreenCapture(monitor=args.monitor) as screen:
            ...
            while not stop_keys.stop_requested and (args.loops <= 0 or completed < args.loops):
                ...
                if not find_and_click(state, mouse, templates["bank_maple_log"], args, args.dry_run):
                    continue
                ...
                if not find_and_click(state, mouse, templates["knife"], args, args.dry_run):
                    continue
